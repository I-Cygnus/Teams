...
function Block({ block, isFirstParagraph }: { block: BlogBlock; isFirstParagraph: boolean }) {
  switch (block.type) {
    case 'h2':
      return (
        <h2 className="mt-16 mb-6 text-[22px] sm:text-[26px] font-bold tracking-[-0.02em] text-[#111] leading-[1.35]">
          {block.text}
        </h2>
      );
    case 'p':
      return (
        <p
          className={`text-[16.5px] sm:text-[17px] text-[#2a2a2a] leading-[1.9] mb-7 ${
    ...
    case 'quote':
      return (
        <blockquote className="my-12 relative pl-7 py-2">
          <span className="absolute left-0 top-0 bottom-0 w-[3px] bg-[#111] rounded-full" />
          <p className="text-[18px] sm:text-[20px] text-[#1a1a1a] leading-[1.6] font-medium tracking-[-0.01em]">
            “{block.text}”
          </p>
        </blockquote>
      );
        ...
        <ul className="my-7 space-y-3">
          {block.items?.map((it, idx) => (
            <li key={idx} className="flex gap-4 text-[16px] text-[#2a2a2a] leading-[1.75]">
              <span
                className="flex-shrink-0 mt-[14px] w-4 h-px bg-[#bbb]"
                aria-hidden
              />
...
function getRelated(current: BlogPost) {
  return blogPosts
    ...
    .sort((a, b) => {
      const aMatch = (a.category === current.category ? 2 : 0) + (a.authorId === current.authorId ? 1 : 0);
      const bMatch = (b.category === current.category ? 2 : 0) + (b.authorId === current.authorId ? 1 : 0);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          scaleX: progress,
          background: author?.accent ?? '#111',
        }}
      />

      {/* Inline drop cap style */}
      <style>{`
          ...
          >
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-[14px] font-semibold text-[#111] tracking-tight group-hover:underline">
                  {author?.name}
                </p>
      ...
      <article className="px-6 pb-24">
        ...
        >
          {post.body.map((b, i) => (
            <Block key={i} block={b} isFirstParagraph={i === firstPIndex} />
          ))}
          ...
          <div className="mt-14 p-7 sm:p-8 rounded-3xl border border-[#eee] bg-white flex flex-col sm:flex-row gap-5 sm:items-center">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-semibold tracking-[0.24em] uppercase text-[#aaa] mb-1.5">
                Author
              </p>
              <p className="text-[16px] font-bold text-[#111] tracking-tight">
                {author?.name} <span className="font-medium text-[#888]">· {author?.role}</span>
              </p>
              <p className="mt-2 text-[13.5px] text-[#666] leading-[1.65]">
                {author?.brief}
              </p>
            </div>
            <Link
        ...
        <section className="px-6 pb-32 border-t border-[#eee] pt-24 bg-[#fafafa]">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((r) => {
                const a = teamMembers.find((m) => m.id === r.authorId);
                return (
                  <Link
                    key={r.id}
                    to={`/blog/${r.id}`}
